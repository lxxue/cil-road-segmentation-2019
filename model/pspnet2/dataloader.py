...
class TrainPre(object):
    ...
    def __call__(self, img, gt, edge, midline):
        img, gt, edge, midline = random_mirror(img, gt, edge, midline) # images are randomly flipped to increase variance
        ...
        if config.train_scale_array is not None:
            img, gt, scale, edge, midline = random_scale(img, gt, config.train_scale_array, edge, midline) # scale the images with supplied list
        ...
        p_edge = cv2.resize(p_edge, (config.image_width // config.gt_down_sampling,
                                     config.image_height // config.gt_down_sampling),
                          interpolation=cv2.INTER_NEAREST)
        p_midline = cv2.resize(p_midline, (config.image_width // config.gt_down_sampling,
                                           config.image_height // config.gt_down_sampling),
                          interpolation=cv2.INTER_NEAREST)
...
class TrainPreOri(TrainPre):
    ...
    def __call__(self, img, gt, edge, midline):
        ...
        midline = img_to_black(midline)
        if config.train_scale_array is not None:
            img, gt, scale, edge, midline = random_scale(img, gt,
                    config.train_scale_array, edge, midline)
        img = normalize(img, self.img_mean, self.img_std)
...
def get_train_loader(engine, dataset):
    data_setting = {'img_root': config.img_root_folder,
                    ...
                    'midline_source': config.midline_source}
